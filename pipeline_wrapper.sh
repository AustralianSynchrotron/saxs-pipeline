...
DAMMIF_SLOW=`qsub -W depend=afterok:$MON_OUTFILE:$POROD_VOLUME -t 0 -v dat_file=$DAT_FILE,output_path=$OUTPUT_PATH,pipeline_home=$PIPELINE_SOURCE_CODE_HOME,mode=slow,prod_ssh_access=$PROD_SSH_ACCESS,prod_scp_dest=$PROD_SCP_DEST,prod_pipeline_harvest=$PROD_PIPELINE_HARVEST,prod_config=$PROD_CONFIG $PIPELINE_SOURCE_CODE_HOME/dammif.pbs -e $OUTPUT_PATH -o $OUTPUT_PATH`
...
DAMMIF_INTERACTIVE=`qsub -W depend=afterok:$MON_INFILE:$POROD_VOLUME -t 1-9 -v dat_file=$DAT_FILE,output_path=$OUTPUT_PATH,pipeline_home=$PIPELINE_SOURCE_CODE_HOME,mode=interactive,prod_ssh_access=$PROD_SSH_ACCESS,prod_scp_dest=$PROD_SCP_DEST,prod_pipeline_harvest=$PROD_PIPELINE_HARVEST,prod_config=$PROD_CONFIG $PIPELINE_SOURCE_CODE_HOME/dammif.pbs -e $OUTPUT_PATH -o $OUTPUT_PATH`
